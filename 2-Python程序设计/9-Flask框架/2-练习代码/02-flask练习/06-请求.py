...
def user_detail():
    ...
    # 参数id 值为10
    # 参数name 值为aaaaa
    # 取得get方式提交的参数 使用request.args.get()
    # 参数值都为字符串
    uid = int(request.args.get('uid'))
    users_info = [{'id': 1, "name": "张三", 'age': 18, 'other': "她喜欢吃吃吃.......... 各种吃，就是不胖，就是玩！"},
                  {'id': 2, "name": "李四", 'age': 19, 'other': "喜欢按摩,各种按，全方位 的按,不怕花钱，就是玩！"},
                  {'id': 3, "name": "王五", 'age': 20, 'other': "喜欢上课，各种课，学习无 止境，别人都以为我在学习，我也不解释"},
                  {'id': 4, "name": "赵六", 'age': 12, 'other': "刷B站，各种刷，网罗全网 各色小姐姐."}, ]
    res = {}
...
def news_detail(nid):
    ...
    return render_template('news_detail.html', **{'t_nid': nid, 'title': "-- 联想大数据班 --"})
